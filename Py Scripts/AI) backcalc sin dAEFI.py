...
def main():
    ...
    plot_config = PlotConfig(
        title_text="Simulate D as modulated sin curves and backcalc threshold of dAEFI", # Title of plot
    ...
    for age_band_pair in age_band_compare:    
        ...
        for idx, age_band in enumerate(age_band_pair):
            ...
            max_days = len(dataframes_dvd[0])
            days = np.linspace(0, max_days-1, max_days) 

            # Define the parameters for each modulated wave, now including horizontal_shift
            modulated_wave_params = [
                {'small_period': 3, 'large_period': 360, 'small_amplitude': 1, 'large_amplitude': 2, 'vertical_shift': 5, 'horizontal_shift': 0},       # Curve 1 D - AG1
                {'small_period': 3, 'large_period': 360, 'small_amplitude': 1, 'large_amplitude': 2, 'vertical_shift': 5, 'horizontal_shift': 0},       # Curve 2 DUVX - AG1
                {'small_period': 3, 'large_period': 360, 'small_amplitude': 1, 'large_amplitude': 2, 'vertical_shift': 5, 'horizontal_shift': 60},      # Curve 3 DVX - AG1
                {'small_period': 3, 'large_period': 360, 'small_amplitude': 1, 'large_amplitude': 2, 'vertical_shift': 5, 'horizontal_shift': 60},      # Curve 3 DVDA - AG1
                {'small_period': 3, 'large_period': 360, 'small_amplitude': 5, 'large_amplitude': 20, 'vertical_shift': 50, 'horizontal_shift': 0},     # Curve 4 D - AG2
                {'small_period': 3, 'large_period': 360, 'small_amplitude': 5, 'large_amplitude': 20, 'vertical_shift': 50, 'horizontal_shift': 0},     # Curve 5 DUVX - AG2
                {'small_period': 3, 'large_period': 360, 'small_amplitude': 5, 'large_amplitude': 20, 'vertical_shift': 50, 'horizontal_shift': 60},    # Curve 6 DVX - AG2
                {'small_period': 3, 'large_period': 360, 'small_amplitude': 5, 'large_amplitude': 20, 'vertical_shift': 50, 'horizontal_shift': 60},      # Curve 3 DVDA - AG2
            ]
            
            # Create all modulated sin curves using the parameters to simulate D, DUVX, DVX Data
            sin_curves = []
            for params in modulated_wave_params:
                sin_curve = create_modulated_wave(
                        days,
                        large_period=params['large_period'],
                        small_period=params['small_period'],
                        large_amplitude=params['large_amplitude'],
                        small_amplitude=params['small_amplitude'],
                        horizontal_shift=params['horizontal_shift'],  # Apply the horizontal shift
                        shift_y=params['vertical_shift']  # Vertical shift (no change)
                    )
            ...
            event_threshold = 5000  # Trigger event after accumulating 10,000 VDAs
            future_day_range = 25  # Random future day range (1 to 25)
            window_size = 7  # Define the window size for the rolling average (adjust this as needed)
            mov_average_vd = dataframes_vd[10][age_band].rolling(window=window_size, min_periods=1).mean()    

            if idx == 0 :
                d_events_without_vda = sin_curves[0]
            if idx == 1 :
                d_events_without_vda = sin_curves[4]
            ...
            for day_idx in range(len(mov_average_vd)):
                ...
                if (cumulative_vda >= event_threshold) :
                    ...
                    if future_idx < len(sin_curves[0]):  # Ensure the index is within bounds for sin_curve[2]
                        # Add the event to sin_curves[2] at the future day index
                        ...
                        if idx == 0 :
                            ...
                            print (f"Idx: {idx} D: {cumulative_vda} cum_D: {d_add}")
                        if idx == 1 :   
                            sin_curves[4][future_idx] += add_d_events                     
                            sin_curves[6][future_idx] += add_d_events
                            sin_curves[7][future_idx] += add_d_events
                            print (f"Idx: {idx} cum_vda: {cumulative_vda} cum_D: {d_add}")

                    # Subtract the event-triggered VDAs from the cumulative VDA count
                    rest = cumulative_vda % event_threshold
                    cumulative_vda = 0  # Event triggered, reset the threshold
            ...
            if idx == 1 : 
                d_events_with_vda = sin_curves[4]  # D events after VDA additions
            ...
            # Optional: Add a breakpoint to inspect the results (for debugging purposes)
            # for example get Estimated Event Threshold: 4981.852988394 if used Threshold was 5000
            # breakpoint()
            ...
            for i in range(0, len(dataframes_dvd)):   
                ...
                if idx == 0 and i == 0:
                    ...
                    dataframes_dvd[i][age_band] = sin_curves[0]  # Curve 1 D
                    dataframes_decay_dvd[i][age_band] = sin_curves[0]
                if idx == 0 and i == 1:
                    leading_zeros = first_nonzero_index(dataframes_dvd[i][age_band])  
                    print (f"idx:{idx} i:{i} AGE:{age_band} zeros:{leading_zeros}")  
                    sin_curves[1] = fill_leading_days_with_zeros(sin_curves[1], leading_zeros)
                    dataframes_dvd[i][age_band] = sin_curves[1]  # Curve 2 DUVX
                    dataframes_decay_dvd[i][age_band] = sin_curves[1]
                if idx == 0 and i == 2:
                    leading_zeros = first_nonzero_index(dataframes_dvd[i][age_band]) 
                    print (f"idx:{idx} i:{i} AGE:{age_band} zeros:{leading_zeros}")
                    sin_curves[2] = fill_leading_days_with_zeros(sin_curves[2], leading_zeros)
                    dataframes_dvd[i][age_band] = sin_curves[2]  # Curve 3 DVX
                    dataframes_decay_dvd[i][age_band] = sin_curves[2]
                if idx == 0 and i == 10:
                    leading_zeros = first_nonzero_index(dataframes_dvd[i][age_band])  
                    print (f"idx:{idx} i:{i} AGE:{age_band} zeros:{leading_zeros}")
                    sin_curves[3] = fill_leading_days_with_zeros(sin_curves[3], leading_zeros)
                    dataframes_dvd[i][age_band] = sin_curves[3]  # Curve 4 DVDA
                    dataframes_decay_dvd[3][age_band] = sin_curves[3]

                if idx == 1 and i == 0:
                    leading_zeros = first_nonzero_index(dataframes_dvd[i][age_band])  
                    print (f"idx:{idx} i:{i} AGE:{age_band} zeros:{leading_zeros}")
                    sin_curves[4] = fill_leading_days_with_zeros(sin_curves[4], leading_zeros)                    
                    dataframes_dvd[i][age_band] = sin_curves[4]  # Curve 5 D
                    dataframes_decay_dvd[i][age_band] = sin_curves[4]
                if idx == 1 and i == 1:
                    leading_zeros = first_nonzero_index(dataframes_dvd[i][age_band])  
                    print (f"idx:{idx} i:{i} AGE:{age_band} zeros:{leading_zeros}")
                    sin_curves[5] = fill_leading_days_with_zeros(sin_curves[5], leading_zeros)                    
                    dataframes_dvd[i][age_band] = sin_curves[5]  # Curve 6 DUVX
                    dataframes_decay_dvd[i][age_band] = sin_curves[5]
                if idx == 1 and i == 2:
                    leading_zeros = first_nonzero_index(dataframes_dvd[i][age_band])  
                    print (f"idx:{idx} i:{i} AGE:{age_band} zeros:{leading_zeros}")
                    sin_curves[6] = fill_leading_days_with_zeros(sin_curves[6], leading_zeros)                    
                    dataframes_dvd[i][age_band] = sin_curves[6]
                    dataframes_decay_dvd[i][age_band] = sin_curves[6]  # Curve 7 DVX
                if idx == 1 and i == 10:
                    leading_zeros = first_nonzero_index(dataframes_dvd[i][age_band])  
                    print (f"idx:{idx} i:{i} AGE:{age_band} zeros:{leading_zeros}")
                    sin_curves[7] = fill_leading_days_with_zeros(sin_curves[7], leading_zeros)                    
                    dataframes_dvd[i][age_band] = sin_curves[7]
                    dataframes_decay_dvd[3][age_band] = sin_curves[7]  # Curve 7 DVDA
...
# Function to determine if an event should occur based on the cumulative VDA value
def should_trigger_event(cumulative_vda_value, event_threshold ):
    return cumulative_vda_value >= event_threshold
